...
export default function PaymentPage() {
  ...
  const expandedItems = orders.flatMap(o =>
    (o.items || []).flatMap((item, ii) => {
      ...
      return Array.from({ length: item.qty }, (_, qi) => ({
        ...
        key: `${o.id}_${ii}_${qi}`,
        paid: paidKeys.includes(`${o.id}_${ii}_${qi}`),
      }))
    })
  )

  const unpaid = expandedItems.filter(i => !i.paid)
  const fullTotal = orders.reduce((s, o) => s + Number(o.total || 0), 0)
  const paidAmt = expandedItems.filter(i => i.paid).reduce((s, i) => s + i.price, 0)
  const remaining = fullTotal - paidAmt
  const splitAmt = unpaid.filter(i => selected[i.key]).reduce((s, i) => s + i.price, 0)
  ...
  const clearTable = async () => {
    ...
    if (!found && tableId) found = (allT||[]).find(t => t.id === tableId)
    if (!found) { setDbg(`Masa tapılmadı. Cəmi ${(allT||[]).length} masa. TN=${n}`); return }

    const todayStr = new Date().toISOString().slice(0,10)
    const nowTime = new Date().toTimeString().slice(0,5)
    const { data: res } = await supabase.from('reservations').select('id,reserved_date,reserved_time')
  ...
  const handlePay = async (method) => {
    ...
    try {
      ...
      const paidKeys = payMode === 'full' ? expandedItems.map(i=>i.key) : unpaid.filter(i=>selected[i.key]).map(i=>i.key)

      await supabase.from('payments').insert({
        business_id: biz.id, customer_token: cTok, customer_name: orders[0]?.customer_name||'',
        amount: payAmount, payment_type: payMode, status: 'paid', gateway: GATEWAY,
        gateway_ref: res.ref, order_ids: orders.map(o=>o.id), paid_item_keys: paidKeys,
      })

      // Hər sifariş üçün paid_item_keys yenilə
      for (const order of orders) {
        const existing = Array.isArray(order.paid_item_keys) ? order.paid_item_keys : []
        const orderPaidKeys = [...new Set([...existing, ...paidKeys.filter(k => k.startsWith(`${order.id}_`))])]

        // Bu sifarişin bütün item×qty key-lərini yoxla
        const totalKeys = (order.items||[]).flatMap((item,ii) =>
          Array.from({length: item.qty}, (_,qi) => `${order.id}_${ii}_${qi}`)
        )
        const allPaid = totalKeys.every(k => orderPaidKeys.includes(k))

        await supabase.from('pending_orders').update({
          paid_item_keys: orderPaidKeys,
          payment_status: allPaid ? 'paid' : 'partial',
        }).eq('id', order.id)
      }

      // Hamısı ödənibsə masanı boşalt
      const allDone = expandedItems.every(i => i.paid || paidKeys.includes(i.key))
      if (allDone) await clearTable()

      setResult(res)
      setStep('done')
    } catch(e) { setResult({ message: e.message }); setStep('error') }
    setProcessing(false)
  }
  ...
  if (step==='done') return (
    <Ctr>
      <div style={{textAlign:'center',maxWidth:380,padding:24}}>
        ...
        <div style={{color:T.text,fontSize:24,fontWeight:800,marginTop:16}}>Ödəniş tamamlandı!</div>
        ...
        {dbg && <div style={{color:dbg.includes('✅')?T.accent:'#FF9F5A',fontSize:11,marginTop:8}}>{dbg}</div>}
      </div>
    </Ctr>
  )
  ...
  if (step==='error') return (
    <Ctr>
      <div style={{textAlign:'center',maxWidth:380,padding:24}}>
        <div style={{fontSize:80}}>❌</div>
        <div style={{color:'#FF5A5F',fontSize:22,fontWeight:800,marginTop:16}}>Ödəniş uğursuz</div>
      ...
      <div style={{width:'100%',maxWidth:440}}>
        ...
        {unpaid.length===0 ? (
              ...
              <div style={{...cBox,marginBottom:16}}>
                {unpaid.map((it,i)=>(
                  <div key={i} style={{display:'flex',justifyContent:'space-between',color:T.sub,fontSize:13,marginBottom:5}}>
                    <span>{it.name}</span><span style={{color:T.text}}>₼{it.price.toFixed(2)}</span>
                  </div>
                ))}
            ...
            {payMode==='split' && (
              <div style={{...cBox,marginBottom:16}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                  <div style={{color:T.sub,fontSize:12}}>Öz payınızı seçin (hər məhsul ayrıca):</div>
                  <button onClick={()=>{const a={};unpaid.forEach(i=>{a[i.key]=true});setSelected(a)}}
                    style={{background:'none',border:'none',color:T.accent,fontSize:12,cursor:'pointer',fontWeight:700}}>Hamısını seç</button>
                </div>
                {unpaid.map((it,idx)=>{
                  const chk=!!selected[it.key]
                  return (
                    <div key={it.key} onClick={()=>setSelected(p=>({...p,[it.key]:!p[it.key]}))}
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{color:T.sub,fontSize:11}}>#{idx+1}</div>
                      </div>
                      <div style={{color:chk?T.accent:T.sub,fontWeight:700,fontSize:14}}>₼{it.price.toFixed(2)}</div>
                    </div>
                  )
                })}
          ...
          {step==='method' && (<>
            ...
            {processing && <div style={{textAlign:'center',color:T.sub,marginTop:16}}>⏳ Emal edilir...</div>}
          ...
          {step==='card' && (<>
            ...
            <div style={cBox}>
              ...
              <div style={{background:'linear-gradient(135deg,#1E2A8A,#2C5BE0)',borderRadius:14,padding:'20px 22px',marginBottom:16}}>
                ...
                <div style={{display:'flex',justifyContent:'space-between',marginTop:14}}>
                  <div><div style={{color:'rgba(255,255,255,0.6)',fontSize:10}}>SAHİBİ</div><div style={{color:'#fff',fontSize:13,fontWeight:600}}>{cardName||'AD SOYAD'}</div></div>
                  <div><div style={{color:'rgba(255,255,255,0.6)',fontSize:10}}>SON TARİX</div><div style={{color:'#fff',fontSize:13,fontWeight:600}}>{cardExp||'MM/YY'}</div></div>
                </div>
              </div>
              ...
              <div style={{display:'flex',gap:10,marginBottom:12}}>
                <div style={{flex:1}}><label style={lbl}>Son istifadə</label><input value={cardExp} onChange={e=>setCardExp(fmtExp(e.target.value))} placeholder="MM/YY" maxLength={5} style={inp}/></div>
                <div style={{flex:1}}><label style={lbl}>CVV</label><input value={cardCvv} onChange={e=>setCardCvv(e.target.value.replace(/\D/g,'').slice(0,3))} placeholder="•••" maxLength={3} type="password" style={inp}/></div>
              </div>
            ...
            <button onClick={()=>handlePay('card')} disabled={processing||!cardNum||!cardExp||!cardCvv||!cardName}
            ...
            <div style={{color:T.sub,fontSize:11,textAlign:'center',marginTop:10}}>🔒 Şifrələnərək ötürülür</div>
...
function Ctr({children}){return<div style={{minHeight:'100vh',background:'#0B1020',color:'#9AA4BC',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'system-ui',fontSize:16}}>{children}</div>}
