...
export default function PaymentPage() {
  ...
  // Ödəniş tipi
  const [payMode, setPayMode] = useState('full')     // full | split
  const [payMethod, setPayMethod] = useState(null)   // card | apple | google

  // Split — seçilmiş məhsullar
  const [selectedItems, setSelectedItems] = useState({}) // { orderId_itemIdx: true }
  ...
  const [step, setStep] = useState('choose')  // choose | method | card | done | error
  ...
  // Bütün məhsulların siyahısı (split üçün)
  const allItems = orders.flatMap((o, oi) =>
    (o.items || []).map((item, ii) => ({ ...item, orderId: o.id, orderIdx: oi, itemIdx: ii, key: `${o.id}_${ii}` }))
  )
  ...
  const payAmount = payMode === 'full' ? fullTotal : selectedTotal
  ...
  const handleChoose = () => {
    if (payMode === 'split' && selectedTotal === 0) return
    setStep('method')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: 'system-ui,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        {orders.length === 0 ? (
          <div style={{ ...cardBox, textAlign: 'center', color: T.sub, padding: 32 }}>
          ...
          <>
            ...
            {step === 'choose' && (
              <>
                ...
                <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                  {[
                    { k: 'full',  icon: '💰', label: 'Hamısını ödə',    desc: `₼${fullTotal.toFixed(2)}` },
                ...
                {/* FULL — sifariş xülasəsi */}
                {payMode === 'full' && (
                  <div style={cardBox}>
                    <div style={{ color: T.sub, fontSize: 12, marginBottom: 10 }}>Bütün sifarişlər:</div>
                    {allItems.map((it, i) => (
                    ...
                    <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 10, marginTop: 8, display: 'flex', justifyContent: 'space-between', color: T.accent, fontWeight: 800, fontSize: 16 }}>
                      <span>Cəmi</span><span>₼{fullTotal.toFixed(2)}</span>
                    </div>
                  </div>
                )}
                    ...
                    {allItems.map((it) => {
                      ...
                      return (
                        <div key={it.key} onClick={() => toggleItem(it.key)}
                          ...
                          <div style={{ color: checked ? T.accent : T.sub, fontWeight: 700, fontSize: 14 }}>
            ...
            {step === 'method' && (
              <>
                <div style={{ ...cardBox, textAlign: 'center', marginBottom: 16 }}>
                  <div style={{ color: T.sub, fontSize: 13 }}>{payMode === 'full' ? 'Ümumi məbləğ' : 'Seçilmiş məbləğ'}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {/* Apple Pay */}
                  <button onClick={() => handleMethod('apple')} disabled={processing}
                  ...
                  {/* Google Pay */}
                  <button onClick={() => handleMethod('google')} disabled={processing}
                    ...
                    <span style={{ fontSize: 24 }}>🇬</span> Google Pay
                  </button>

                  {/* Kart */}
                  <button onClick={() => handleMethod('card')} disabled={processing}
                ...
                {processing && (
                  <div style={{ textAlign: 'center', color: T.sub, marginTop: 16, fontSize: 13 }}>
                    ⏳ Emal edilir...
                  </div>
                )}

                <button onClick={() => setStep('choose')}
                  style={{ width: '100%', marginTop: 14, padding: 12, borderRadius: 12, border: `1px solid ${T.border}`, background: 'transparent', color: T.sub, cursor: 'pointer', fontSize: 14 }}>
                  ← Geri
                </button>
              </>
            )}
              ...
              <>
                ...
                <div style={cardBox}>
                  ...
                  <div style={{ background: 'linear-gradient(135deg,#1E2A8A,#2C5BE0)', borderRadius: 14, padding: '20px 22px', marginBottom: 16 }}>
                    ...
                    <div style={{ color: '#fff', fontSize: 20, fontWeight: 700, letterSpacing: 3 }}>
                ...
                <div style={{ color: T.sub, fontSize: 11, textAlign: 'center', marginTop: 10 }}>
                  🔒 Məlumatlarınız şifrələnərək ötürülür
                </div>

                <button onClick={() => setStep('method')}
                  style={{ width: '100%', marginTop: 10, padding: 12, borderRadius: 12, border: `1px solid ${T.border}`, background: 'transparent', color: T.sub, cursor: 'pointer', fontSize: 14 }}>
                  ← Geri
                </button>
              </>
            )}
...
const cardBox = { background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 18, marginBottom: 0 }
